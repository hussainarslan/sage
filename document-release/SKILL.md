...
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.sage/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not sage-specific):
...
```bash
gh pr view --json body -q .body > /tmp/sage-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/sage-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/sage-pr-body-$$.md
```
