...
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/sage/browse/dist/browse" ] && B="$_ROOT/.claude/skills/sage/browse/dist/browse"
[ -z "$B" ] && B=~/.claude/skills/sage/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
4. **List existing design docs for this project:**
   ```bash
   ls -t ~/.sage/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.sage/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.sage/projects/`.
...
```bash
PRIOR=$(ls -t ~/.sage/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.sage/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
```bash
mkdir -p ~/.sage/analytics
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.sage/analytics/spec-review.jsonl 2>/dev/null || true
```
...
- If yes → run `open https://ycombinator.com/apply?ref=sage` and say: "Bring this design doc to your YC interview. It's better than most pitch decks."
...
>
> **ycombinator.com/apply?ref=sage**
...
The design doc at `~/.sage/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
