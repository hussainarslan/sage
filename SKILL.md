...
description: |
  ...
  Also suggest adjacent sage skills by stage: brainstorm /office-hours; strategy
  ...
  /retro; research /research; quality improvement /ralph; second opinion /codex; prod
  safety /careful or /guard; scoped edits /freeze or /unfreeze; sage upgrades
  /sage-upgrade. If the user opts out of suggestions, stop and run sage-config set
  proactive false; if they opt back in, run sage-config set proactive true.
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
echo '{"skill":"sage","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
...
If `PROACTIVE` is `false`: do NOT proactively suggest other sage skills during this session.
Only run skills the user explicitly invokes. This preference persists across sessions via
`sage-config`.

# sage browse: QA Testing & Dogfooding
