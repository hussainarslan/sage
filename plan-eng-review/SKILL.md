...
Write to `~/.sage/projects/{slug}/{user}-{branch}-test-plan-{datetime}.md`:
...
```bash
~/.claude/skills/sage/bin/sage-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
- **C)** Run /research (only if the plan touches unfamiliar technology or domains that need investigation)
