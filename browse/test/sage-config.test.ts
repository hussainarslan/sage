/**
 * Tests for bin/sage-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * SAGE_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'sage-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'sage-config-test-'));
});
...
describe('sage-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { SAGE_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
