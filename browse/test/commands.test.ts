...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/sage-cli-');
    const execPath = path.join(root, '.claude/skills/sage/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/sage/browse/src/server.ts');
