...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/sage/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.sage/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/sage/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.sage/projects/$REMOTE_SLUG"
mkdir -p ~/.sage
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.sage/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.sage/greptile-history.md` (global aggregate)
