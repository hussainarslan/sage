...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
HANDOFF=$(ls -t ~/.sage/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
`~/.claude/skills/sage/office-hours/SKILL.md`
...
```bash
eval "$(~/.claude/skills/sage/bin/sage-slug 2>/dev/null)" && mkdir -p ~/.sage/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.sage/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.sage/projects/$SLUG/ceo-plans/{old-plan}.md ~/.sage/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.sage/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```bash
mkdir -p ~/.sage/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.sage/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + sage,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+sage: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
rm -f ~/.sage/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.sage/` (user config directory, not project files). The skill preamble
already writes to `~/.sage/sessions/` and `~/.sage/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.claude/skills/sage/bin/sage-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.sage/projects/` only (local, personal reference)
- **C)** Skip
