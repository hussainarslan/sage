...
```bash
source <(~/.claude/skills/sage/bin/sage-slug 2>/dev/null) && mkdir -p ~/.sage/projects/$SLUG/ceo-plans
```
...
```bash
~/.claude/skills/sage/bin/sage-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
- **C)** Run /research (only if accepted expansions touch unfamiliar domains that need investigation)
