...
const HOST_PATHS: Record<Host, HostPaths> = {
  claude: {
    skillRoot: '~/.claude/skills/sage',
    localSkillRoot: '.claude/skills/sage',
    binDir: '~/.claude/skills/sage/bin',
    browseDir: '~/.claude/skills/sage/browse/dist',
  },
  codex: {
    skillRoot: '$SAGE_ROOT',
    localSkillRoot: '.agents/skills/sage',
    binDir: '$GSTACK_BIN',
...
/** sage's 10 AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES */
...
SAGE_ROOT="$HOME/.codex/skills/sage"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/sage" ] && SAGE_ROOT="$_ROOT/.agents/skills/sage"
GSTACK_BIN="$SAGE_ROOT/bin"
GSTACK_BROWSE="$SAGE_ROOT/browse/dist"
`
    : '';
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/sage-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/sage-update-check 2>/dev/null || true)
...
_CONTRIB=$(${ctx.paths.binDir}/sage-config get sage_contributor 2>/dev/null || true)
_PROACTIVE=$(${ctx.paths.binDir}/sage-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
source <(${ctx.paths.binDir}/sage-repo-mode 2>/dev/null) || true
REPO_MODE=\${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.sage/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${ctx.paths.binDir}/sage-config get telemetry 2>/dev/null || true)
...
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
# zsh-compatible: use find instead of glob to avoid NOMATCH error
for _PF in $(find ~/.sage/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do [ -f "$_PF" ] && ${ctx.paths.binDir}/sage-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
\`\`\``;
}

function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest sage skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows \`UPGRADE_AVAILABLE <old> <new>\`: read \`${ctx.paths.skillRoot}/sage-upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If \`JUST_UPGRADED <from> <to>\`: tell user "Running sage v{to} (just updated!)" and continue.`;
}
...
If A: run \`${ctx.paths.binDir}/sage-config set telemetry community\`
...
If B→A: run \`${ctx.paths.binDir}/sage-config set telemetry anonymous\`
If B→B: run \`${ctx.paths.binDir}/sage-config set telemetry off\`
...
If \`NEEDS_SETUP\`:
1. Tell the user: "sage browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
Check if the diff touches frontend files using \`sage-diff-scope\`:

\`\`\`bash
source <(${ctx.paths.binDir}/sage-diff-scope <base> 2>/dev/null)
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/sage-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
\`\`\`
...
**Local:** \`.sage/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md\`

**Project-scoped:**
\`\`\`bash
eval "$(~/.claude/skills/sage/bin/sage-slug 2>/dev/null)" && mkdir -p ~/.sage/projects/$SLUG
\`\`\`
Write to: \`~/.sage/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md\`
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \\\`sage-config set skip_eng_review true\\\` (the "don't bother me" setting).
...
[ -f .sage/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
\`\`\`
...
If user picks H → write \`.sage/no-test-bootstrap\` and continue without tests.
...
If user picks C → write \`.sage/no-test-bootstrap\`. Tell user: "If you change your mind later, delete \`.sage/no-test-bootstrap\` and re-run." Continue without tests.
...
Write to \`~/.sage/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md\`:
...
Write to \`~/.sage/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md\`:
...
\`\`\`bash
mkdir -p ~/.sage/analytics
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.sage/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
\`~/.claude/skills/sage/${first}/SKILL.md\`
...
\`\`\`bash
SLUG=$(~/.claude/skills/sage/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.sage/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.sage/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
\`\`\`bash
SKETCH_FILE="/tmp/sage-sketch-$(date +%s).html"
\`\`\`
...
$B screenshot /tmp/sage-sketch.png
\`\`\`
...
The screenshot file at \`/tmp/sage-sketch.png\` can be referenced by downstream skills
...
\`\`\`bash
CODEX_PROMPT_FILE=$(mktemp /tmp/sage-codex-oh-XXXXXXXX.txt)
\`\`\`
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(~/.claude/skills/sage/bin/sage-config get codex_reviews 2>/dev/null || true)
...
\`\`\`bash
~/.claude/skills/sage/bin/sage-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
\`\`\`bash
~/.claude/skills/sage/bin/sage-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
**Persist the result:**
\`\`\`bash
~/.claude/skills/sage/bin/sage-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/sage-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
Replace STATUS with "clean" or "issues_found", SOURCE with "codex+subagent", "codex-only", "subagent-only", or "unavailable".`;
}

// ─── Design Hard Rules (OpenAI framework + sage slop blacklist) ───
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + sage design methodology.`;
}

function generateSlugEval(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/sage-slug 2>/dev/null)"`;
}

function generateSlugSetup(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/sage-slug 2>/dev/null)" && mkdir -p ~/.sage/projects/$SLUG`;
}
...
function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'sage';
  // Don't double-prefix: sage-upgrade → sage-upgrade (not sage-sage-upgrade)
  if (skillDir.startsWith('sage-')) return skillDir;
  return `sage-${skillDir}`;
}
...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string } {
  ...
  if (host === 'codex') {
    ...
    content = content.replace(/~\/\.claude\/skills\/sage/g, ctx.paths.skillRoot);
    content = content.replace(/\.claude\/skills\/sage/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.claude\/skills\/review/g, '.agents/skills/sage/review');
