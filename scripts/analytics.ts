#!/usr/bin/env bun
/**
 * analytics — CLI for viewing sage skill usage statistics.
 *
 * Reads ~/.sage/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.sage', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('sage skill usage analytics');
