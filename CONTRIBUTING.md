...
sage skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/sage/` (your global install). But when you're developing sage itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
Contributor mode turns sage into a self-improving tool. Enable it and Claude Code
will periodically reflect on its sage experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.sage/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/sage/bin/sage-config set sage_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork sage, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use sage normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.sage/contributor-logs/`
3. **Fork and clone sage** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where sage annoyed you)
   ln -sfn /path/to/your/sage-fork .claude/skills/sage
   cd .claude/skills/sage && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using sage** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix sage while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ sage sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on sage inside the sage repo

When you're editing sage skills and want to test them by actually using sage
...
```
sage/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── sage -> ../../         <- symlink back to repo root
│   ├── review -> sage/review
│   ├── ship -> sage/ship
...
When E2E tests run, they produce machine-readable artifacts in `~/.sage-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.sage-dev/` for post-mortem debugging and trend analysis.
...
sage generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/sage-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/sage` | `$SAGE_ROOT` (`.agents/skills/sage` in a repo, otherwise `~/.codex/skills/sage`) |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/sage`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop sage.** Symlink your sage checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/sage-checkout .claude/skills/sage
cd .claude/skills/sage && bun install && bun run build
```

Now every sage skill invocation in this project uses your working tree. Edit a
...
```bash
rm .claude/skills/sage
```

Claude Code falls back to `~/.claude/skills/sage/` automatically.
