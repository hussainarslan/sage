...
This document covers the command reference and internals of sage's headless browser.
...
sage's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.sage/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.sage/browse-console.log`
- Network: `.sage/browse-network.log`
- Dialog: `.sage/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.sage/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.sage/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.sage/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.sage/browse.json` | Path to state file (CLI passes to server) |
...
| **sage browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. sage burns zero.
...
sage skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.sage/browse.json`, sends HTTP to the server, prints response. |
...
The active skill lives at `~/.claude/skills/sage/`. After making changes:

1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/sage && git pull`
3. Rebuild: `cd ~/.claude/skills/sage && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/sage/browse/dist/browse`
