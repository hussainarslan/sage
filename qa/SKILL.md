...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.sage/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
| Mode | full | `--regression .sage/qa-reports/baseline.json` |
| Output dir | `.sage/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .sage/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.sage/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   source <(~/.claude/skills/sage/bin/sage-slug 2>/dev/null)
   ls -t ~/.sage/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .sage/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.sage/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
source <(~/.claude/skills/sage/bin/sage-slug 2>/dev/null) && mkdir -p ~/.sage/projects/$SLUG
```
Write to `~/.sage/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
