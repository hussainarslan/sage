...
// Eval result collector — accumulates test results, writes to ~/.sage-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.sage', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/sage/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with sage installed globally, the fallback path
    // ~/.claude/skills/sage/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.sage/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a sage issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey sage team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a sage skill. The session preamble detected _SESSIONS=4 (the user has 4 sage windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.sage/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.sage', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.sage/projects/', async () => {
...
describeIfSelected('sage-upgrade E2E', ['sage-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sage-remote-'));
    ...
    // Create mock sage install directory (local-git type)
    const mockSage = path.join(upgradeDir, '.claude', 'skills', 'sage');
    fs.mkdirSync(mockSage, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockSage);
    run('git', ['config', 'user.email', 'test@test.com'], mockSage);
    run('git', ['config', 'user.name', 'Test'], mockSage);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockSage);

    // Write old version files
    fs.writeFileSync(path.join(mockSage, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockSage, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockSage, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockSage);
    run('git', ['commit', '-m', 'initial'], mockSage);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockSage);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockSage, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockSage, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockSage);
    run('git', ['commit', '-m', 'release 0.6.0'], mockSage);
    run('git', ['push', 'origin', 'HEAD:main'], mockSage);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockSage);

    // Copy sage-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'sage-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'sage-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'sage-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('sage-upgrade-happy-path', async () => {
    const mockSage = path.join(upgradeDir, '.claude', 'skills', 'sage');
    const result = await runSkillTest({
      prompt: `Read sage-upgrade/SKILL.md for the upgrade workflow.

You are running /sage-upgrade standalone. The sage installation is at ./.claude/skills/sage (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/sage — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'sage-upgrade-happy-path',
      runId,
    });

    logCost('/sage-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockSage, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/sage-upgrade happy path', 'sage-upgrade E2E', result, {
