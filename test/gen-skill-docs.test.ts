...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root sage' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('sage_contributor');
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.sage/analytics');
  });
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find ~/.sage/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('sage-review');
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('GSTACK REVIEW REPORT');
    expect(content).toContain('sage-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'sage-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('sage-codex-oh-');
  });
});
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      skills.push({ dir: '.', codexName: 'sage' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue; // /codex is excluded from Codex output
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const codexName = entry.name.startsWith('sage-') ? entry.name : `sage-${entry.name}`;
  ...
  test('root sage bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "sage"');
    expect(content).toContain('Use $sage to locate the bundled sage skills.');
  });

  test('codexSkillName mapping: root is sage, others are sage-{dir}', () => {
    // Root → sage
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage', 'SKILL.md'))).toBe(true);
    // Subdirectories → sage-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage-ship', 'SKILL.md'))).toBe(true);
    // sage-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: sage-sage-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage-sage-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'sage-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'sage-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'sage-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'sage-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['sage-careful', 'sage-freeze', 'sage-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global sage roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'sage-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('GSTACK_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/sage');
    expect(content).toContain('$SAGE_BIN/sage-config');
    expect(content).toContain('$SAGE_ROOT/sage-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/sage/bin/sage-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/sage/review/ (not sage-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/sage-review
    // but setup puts sidecars under .agents/skills/sage/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'sage-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use sage/review/ path
    expect(content).toContain('.agents/skills/sage/review/checklist.md');
    expect(content).toContain('.agents/skills/sage/review/design-checklist.md');
    // Wrong: must NOT reference sage-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/sage-review/checklist.md');
    expect(content).not.toContain('.agents/skills/sage-review/design-checklist.md');
  });

  test('sidecar paths in ship skill point to sage/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'sage-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/sage/review/');
      expect(content).not.toContain('.agents/skills/sage-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/sage/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/sage-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/sage → $SAGE_ROOT
    expect(content).not.toContain('~/.claude/skills/sage');
    expect(content).toContain('$SAGE_ROOT');

    // Rule 2: .claude/skills/sage → .agents/skills/sage
    expect(content).not.toContain('.claude/skills/sage');

    // Rule 3: .claude/skills/review → .agents/skills/sage/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('sage-config') || content.includes('sage-update-check') || content.includes('sage-telemetry-log')) {
        expect(content).toContain('$SAGE_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('sage-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/sage');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of ALL_SKILLS) {
      const content = fs.readFileSync(path.join(ROOT, skill.dir, 'SKILL.md'), 'utf-8');
      expect(content).not.toContain('~/.codex/');
      // sage-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'sage-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'sage-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'sage-ship', 'SKILL.md'), 'utf-8');
...
describe('setup script validation', () => {
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('sage*');
  });

  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "sage/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "sage/$skill_name"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/sage from being linked as a skill
  test('link_codex_skill_dirs skips the sage sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "sage" ] && continue');
  });

  // T2: Dynamic $SAGE_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic GSTACK_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'sage-ship');
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/sage');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('sage/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('sage-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$sage_dir" "$codex_sage"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/sage', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.sage/repos/sage');
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('sage-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help sage get better');
    expect(content).toContain('sage-config set telemetry community');
    expect(content).toContain('sage-config set telemetry anonymous');
    expect(content).toContain('sage-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('sage-telemetry-log');
